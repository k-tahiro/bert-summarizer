...
class BeamSearch:
    ...
    def initialize(self):
        self.alive_seq = torch.full([self.beam_batch_size, 1],
                                    ...
                                    dtype=torch.long,
                                    device=self.device)
        self.topk_log_probs = torch.full([self.beam_batch_size, 1],
                                         float('-inf'),
                                         device=self.device)
        self.topk_log_probs[self.beam_offset] = 0.0

    def get_topk_ids(self, log_probs: torch.Tensor) -> torch.Tensor:
    ...
    def length_penalty(self):
        return ((5.0 + (self.step + 1)) / 6.0) ** self.alpha
    ...
    def _run(self,
        ...
        for src, _ in data_loader:
            ...
            for step in range(max_length):
                tgt = self.model.embeddings(beam_search.alive_seq[:, -1])
                ...
                if step < min_length:
                    log_probs[:, self.eos_token_id] = -1e20

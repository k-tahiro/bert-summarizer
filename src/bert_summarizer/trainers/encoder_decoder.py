...
class EncoderDecoderTrainer(Trainer):
        ...
        **kwargs: Any,
        ...
        if not self.is_given_optims:
            encoder_params = self._get_params(
                model.encoder, args.weight_decay, encoder_learning_rate
            )
            decoder_params = self._get_params(
                model.decoder, args.weight_decay, decoder_learning_rate
            )
            ...
            lr_scheduler = get_transformer_schedule_with_warmup(
                optimizer, [encoder_warmup_steps] * 2 + [decoder_warmup_steps] * 2
            )
    ...
    @classmethod
    def _get_params(
        cls, model: nn.Module, weight_decay: float, learning_rate: float
    ) -> list:
        return [
            {
                "params": [
